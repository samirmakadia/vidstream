...
class _OtherUserProfileScreenState extends State<OtherUserProfileScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: 1, vsync: this); // Only show user's videos, not liked videos
  ...
  Future<void> _loadUserProfile() async {
    try {
      ...
      try {
        videos = await ApiRepository.instance.videos.getUserVideos(widget.userId);
      } catch (e) {
  ...
  Future<void> _toggleFollow() async {
    final currentUserId = ApiRepository.instance.auth.currentUser?.id;
    if (currentUserId == null || currentUserId == widget.userId || _isBlocked) return;
  ...
  Future<void> _toggleBlock() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_isBlocked ? 'Blocked ${_user?.displayName}' : 'Unblocked ${_user?.displayName}'),
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: CircleAvatar(
                radius: 46,
                backgroundImage: _user?.profileImageUrl != null || _user?.photoURL != null
