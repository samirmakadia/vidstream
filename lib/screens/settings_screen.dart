...
// EditProfileBottomSheet - moved from profile_screen.dart for reuse
...
class _EditProfileBottomSheetState extends State<EditProfileBottomSheet> {
  ...
  Future<String?> _uploadImage(XFile imageFile, String folder) async {
    try {
      final userId = widget.currentUser?.id ?? 'anonymous';
      final fileName = '${DateTime.now().millisecondsSinceEpoch}_${imageFile.name}';
      // TODO: Replace with API-based image upload
      // For now, return a placeholder URL
      await Future.delayed(const Duration(seconds: 1)); // Simulate upload delay
      return 'https://via.placeholder.com/400x400.png?text=Uploaded+Image';
    } catch (e) {
  ...
  Future<void> _pickImage(String type) async {
    try {
      ...
      if (image != null) {
        ...
        final imageUrl = await _uploadImage(image, '${type}s');
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (userId != null) {
        ...
        print('Selected DOB: $_selectedDateOfBirth');
